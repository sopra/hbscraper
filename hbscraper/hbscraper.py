...
class hbscraper(object):
  ...
  def get_reviewpage(self, shop, url, page):
    ...
    # 掲載終了ならつぎへ
    content = soup.find(id = 'contents').find('div', attrs = {'class', 'contentsTitleWrapper'})
    if content is not None:
      logging.info('This salon has finished to publish.')
      return
    ...
    # 「次へ」があれば次のページへ
    prelist = soup.find('div', attrs={'class', 'preList'}).find(
        'p', attrs={'class', 'pa bottom0 right0'})
  ...
  def parse(self, soup, dom, shop):
    ...
    for ritem in rv_items:
      ...
      # response
      shop_response = ritem.find(
          'div', attrs={'class', 'mT20 mH10 pV5 pH9 bdGray'})
      if shop_response:
        record.append(shop_response.find(
            'p', attrs={'class', 'mT10 wwbw'}).get_text())
      else:
        record.append('')
